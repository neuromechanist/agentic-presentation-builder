/**
 * Shared HTML renderer factory.
 * Inject markdown rendering so browser and Node builds can share slide markup logic.
 * @param {Function} markdownToHtml - Markdown to HTML renderer
 * @returns {{ renderPresentation: Function }}
 */
export function createPresentationRenderer(markdownToHtml) {
  /**
  ...
  function renderSlide(slide, index, total) {
    ...
    const slideTitleAttr = slide.title
  ...
  function renderTextElement(element) {
    ...
    const classes = [
      "text-element",
      getSizeClass(element.style.fontSize),
      ...animation.classes,
    ].join(" ");
    const attrs = [styleAttr, animation.attributes]
  ...
  function renderBulletsElement(element) {
    ...
    const items = element.items
      .map((item) => renderBulletItem(item))
      .join("\n");

    const classes = [
      "bullet-list",
      `style-${element.bulletStyle}`,
      getSizeClass(element.style.fontSize),
      ...animation.classes,
    ].join(" ");
    const attrs = [styleAttr, animation.attributes]
  ...
  function renderBulletItem(item) {
    ...
    const classAttr =
      animation.classes.length > 0
        ? ` class="${animation.classes.join(" ")}"`
        : "";
  ...
  function renderImageElement(element) {
    ...
    const widthStyle =
      element.width !== "auto" ? `width: ${element.width};` : "";
    const heightStyle =
      element.height !== "auto" ? `height: ${element.height};` : "";
    const frameStyle =
      widthStyle || heightStyle ? ` style="${widthStyle}${heightStyle}"` : "";
    const animation = buildAnimationAttributes(element.animation);
